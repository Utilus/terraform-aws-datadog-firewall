...
resource "null_resource" "extract_log_ip_list" {
  ...
  provisioner "local-exec" {
    command = "cat ${local.datadog_ip_ranges_file} | jq -r '.logs .prefixes_ipv4[] | map(select(length > 0))' | shuf | head -n ${local.security_group_rule_limit} | tee ${local.log_ips_file}"
  }
