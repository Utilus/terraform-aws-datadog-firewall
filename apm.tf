...
resource "null_resource" "extract_apm_ip_list" {
  ...
  provisioner "local-exec" {
    command = "cat ${local.datadog_ip_ranges_file} | jq -r '.apm .prefixes_ipv4[] | map(select(length > 0))' | shuf | head -n ${local.security_group_rule_limit} | tee ${local.apm_ips_file}"
  }
